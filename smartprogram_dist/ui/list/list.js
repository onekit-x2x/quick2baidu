...
var checkDeviceWidth = function checkDeviceWidth() {
  var info = swan.getSystemInfoSync();
...
Component({
    ...
    '../list-item/list-item': {
      ...
      linked: function linked(target) {
        ...
        if (!this._hasCheckSize) {
          ...
          setTimeout(function () {
            try {
              target.createSelectorQuery().select('.swan-recycle-item').boundingClientRect(function (rect) {
  ...
  methods: {
    _log: function _log() {
      ...
      for (var _len = params.length, _args = Array(_len), _key = 0; _key < _len; _key++) {
        _args[_key] = params[_key];
      }
    ...
    _calcViewportIndexes: function _calcViewportIndexes(left, top, cb) {
      ...
      this._getBeforeSlotHeight(function () {
        ...
        var beginIndex = _that$__calcViewportI.beginIndex;
        var endIndex = _that$__calcViewportI.endIndex;
        var minTop = _that$__calcViewportI.minTop;
        var afterHeight = _that$__calcViewportI.afterHeight;
        var maxTop = _that$__calcViewportI.maxTop;
    ...
    reRender: function reRender(cb) {
      ...
      var beforeSlotHeight = 0;
      var afterSlotHeight = 0;
      var that = this;
