...
Page({
  ...
  create(name, isbuffer) {
    ...
    //   url: isbuffer === true ? 'ws://demos.kaazing.com/echo' : 'wss://echo.websocket.org',
      url: isbuffer === true ? 'ws://www.onekitwx.com/quick/weixin/websocket/websocket2' : 'ws://www.onekitwx.com/quick/weixin/websocket/websocket1',
      header: {
  ...
  send1(){
    this.send("websocket1");
  },
  send2(){
    this.send("websocket2");
  },
  send(name) {
    // var name = "websocket1";
    let _ws = ws[name]
    ...
    _ws.send({
      ...
      success: function () {
        ...
        // })
        console.log(`[client ${name}] send success`);
      },
  ...
  close1(){
    this.close("websocket1");
  },
  close2(){
    this.close("websocket2");
  },
  ...
  close(name) {
    ...
    ws[name].close({
      success: function () {
        ...
        // })
        console.log(`[client ${name}] close success`);
        ws[name] = null
      },
      fail: function (data, code) {
        // prompt.showToast({
